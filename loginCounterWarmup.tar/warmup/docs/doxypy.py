...
class Doxypy(object):
    ...
    def makeCommentBlock(self):
        ...
        # Reset the indentation of all lines to the one with smallest indentation
        minIndent = None
        for l in commentLines:
            m = re.search(r'\S', l) # First non-space
            if m:
                thisIndent = m.start(0)
                if minIndent is None or thisIndent < minIndent:
                    minIndent = thisIndent
        if minIndent is None:
            minIndent = 0
        commentLines = map(lambda x: "%s# %s" % (self.indent, x[minIndent:]), commentLines)
