...
def home(request):
    ...
    if request.user.isauthenticated() and "User has already registered placeholder for db call":
        return render(request, 'dashboard.html', {'this needs db data':'TODO'})
...
def dashboard(request):
    ...
    if not request.user.isauthenticated():
        return render(request, 'home.html',RequestContext(request,{}))
    #TODO: If user has not registered yet (profile does not exist) 
    if not "User registered placeholder call, probably a get to the user account DB":
        return render(request, 'register.html',RequestContext(request,{}))
    else:
