...
def dashboardData(request):
    ...
    for planner in range(len(userInformation['planners'])):
        if userInformation['planners'][planner]:
            newPlanner = []            
            for course in userInformation['planners'][planner]:
                try:
                    newPlanner.append(dbToReadable(course))
                except:
                    newPlanner.append(course)
            readablePlanner.append(newPlanner)
        else:
            readablePlanner.append([])
    userInformation['planners'] = readablePlanner
    unitCount = []
